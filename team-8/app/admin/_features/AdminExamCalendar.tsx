...
> = {
  ...
  completed: {
    label: "Дууссан",
    pillClassName: "bg-[#E05252] text-white",
    eventClassName: "bg-[#d8f2da] text-[#367c3c]",
  },
  draft: {
    label: "Ноорог",
    pillClassName: "bg-[#F2B544] text-white",
...
function formatWeekday(dateKey: string) {
  ...
  return WEEKDAY_LONG_LABELS[weekdayIndex] ?? "";
}
...
export default function AdminExamCalendar({
  ...
  const monthGrid = useMemo(
    () => buildMonthGrid(visibleMonthKey),
    [visibleMonthKey],
  );
  const weekGrid = useMemo(
    () => buildWeekGrid(selectedDateKey),
    [selectedDateKey],
  );
  ...
  return (
    <div className="flex flex-col gap-7.5">
      <div className="rounded-[28px] border-zinc-200 shadow-none">
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl pt-1.5 w-full">
            <div className="bg-white rounded-2xl  w-full max-w-360">
              <div className="flex items-center gap-1.5 mb-1.5 px-2.5 ">
                {/* Search */}
                <div className="flex items-center gap-1 pl-2">
                  <Search size={16} className="text-[#8F929C]" />
                  <input
                    type="text"
                    placeholder="Хайх"
                    className="h-7 pl-1 w-13 text-[13px]"
                  />
                </div>
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="h-7 rounded-lg border-zinc-200 px-2 text-[13px]"
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-transparent transition-colors hover:bg-zinc-100"
              ...
              {view === "month" ? (
                <div className="overflow-x-auto">
                  <div className="grid grid-cols-7 border-x border-b border-zinc-200 min-w-[560px]">
                    {monthGrid.map((dateKey, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "min-h-[92px] border-r border-t border-zinc-200 px-2.5 py-2.5 text-left align-top transition-colors",
                            index % 7 === 6 ? "border-r-0" : "",
                            !isCurrentMonth && "bg-zinc-50/60 text-zinc-300",
                            isSelected && "bg-[#f5f9ff]",
                            isToday &&
                              "shadow-[inset_0_0_0_1px_rgba(47,128,237,0.18)]",
                          )}
                        >
                          <div className="flex h-full flex-col">
                            ...
                            <div className="mt-2.5 space-y-1.5">
                              ...
                              {dayExams.length > 1 ? (
                                <p className="px-1 text-xs font-medium text-[#4A5565]">
                  ...
                  {weekGrid.map((dateKey) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => setSelectedDateKey(dateKey)}
                        className={cn(
                          "min-h-[180px] rounded-[20px] border border-zinc-200 px-3 py-3 text-left transition-colors",
                          isSelected
                            ? "bg-[#f5f9ff] shadow-[inset_0_0_0_1px_rgba(47,128,237,0.2)]"
                            : "bg-white",
                          ...
                          {dayExams.length > 0 ? (
                            dayExams.slice(0, 2).map((exam) => (
                              <div key={exam.id} className="space-y-1">
                                ...
                                <p className="px-1 text-[11px] text-[#4A5565]">
                                  {formatTimeLabel(exam.start_time)} -{" "}
                                  {formatTimeLabel(exam.end_time)}
                                </p>
                              </div>
                            ))
                          ) : (
                            <p className="pt-10 text-xs text-zinc-300">
                              Шалгалтгүй
                            </p>
                          )}
                        </div>
                ...
                <div className="space-y-3">
                  {selectedDateExams.length > 0 ? (
                    selectedDateExams.map((exam) => (
                      <div
                        key={exam.id}
                        className="flex items-center justify-between rounded-[20px] border border-zinc-200 px-3.5 py-3.5"
                      >
                    ...
                    <div className="rounded-[24px] border border-dashed border-zinc-200 px-4 py-10 text-center text-sm text-[#4A5565]">
        ...
        <div className="mt-4 space-y-3">
          {selectedDateExams.length > 0 ? (
            selectedDateExams.map((exam) => (
              <div
                key={exam.id}
                className="rounded-2xl border bg-white shadow-md border-zinc-200 px-5 py-4"
              >
                ...
                {exam.subject_name ? (
                  <p className="mt-1 text-sm text-[#4A5565]">
                    Шалгалт хариуцсан багш: {exam.subject_name}
                  </p>
                ) : null}

                <div className="mt-3 space-y-1.5 text-sm text-[#4A5565]">
                  <div className="flex items-center gap-2">
                    <Users2 className="h-4 w-4 text-[#4A5565]" />
                    <span>Бүлэг: {getGroupsLabel(exam.groups)}</span>
                  </div>

                  <div className="flex items-center gap-2">
                    <Clock3 className="h-4 w-4 text-[#4A5565]" />
                    <span>
                      {formatTimeLabel(exam.start_time)} -{" "}
                      {formatTimeLabel(exam.end_time)}
                    </span>
            ...
            <div className="rounded-2xl border border-dashed border-zinc-200 px-4 py-7 text-sm text-[#4A5565]">
