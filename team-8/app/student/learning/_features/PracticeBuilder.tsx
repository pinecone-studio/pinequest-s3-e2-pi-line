...
export default function PracticeBuilder({
  ...
  const handleCreatePractice = (targetTopicKeys: string[]) => {
    startTransition(async () => {
      setError(null);
      const result = await createStudentPracticeExam({
        subjectId,
        topicKeys: targetTopicKeys,
      });
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-6">
          <TopicSelectorCard
            ...
            onCreatePractice={() => handleCreatePractice(selectedTopicKeys)}
          />
        </div>
      </section>
        ...
        <SectionHeader
          ...
          trailing={
            <select
              ...
              className="h-10 rounded-[10px] border border-[#D7D7D7] bg-white px-4 text-[13px] font-medium text-[#111111] outline-none"
            >
              <option value="all">Бүх хичээл</option>
              <option value="selected">{subjectName}</option>
            </select>
          }
        />
